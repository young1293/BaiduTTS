package com.lxq.baidutts.control;
...
import com.lxq.baidutts.MainHandlerConstant;
...
public class MySyntherizer implements MainHandlerConstant {
    ...
    protected MySyntherizer(Context context) {
        if (isInitied) {
            Log.e("----MySyntherizer-----", "MySynthesizer 类里面 SpeechSynthesizer还未释放，请勿新建一个新类");
    ...
    protected boolean init(InitConfig config) {
        ...
        if (isMix) {
            ...
            if (!authInfo.isSuccess()) {
                ...
                Log.e("----MySyntherizer-----", "鉴权失败 =" + errorMsg);
                //sendToUiThread("鉴权失败 =" + errorMsg);
                return false;
            } else {
                Log.e("----MySyntherizer-----", "验证通过，离线正式授权文件存在。");
        ...
        if (result != 0) {
            Log.e("----MySyntherizer-----", "【error】initTts 初始化失败 + errorCode：" + result);
            //sendToUiThread("【error】initTts 初始化失败 + errorCode：" + result);
            return false;
        }
        // 此时可以调用 speak和synthesize方法
        Log.e("----MySyntherizer-----", "合成引擎初始化成功");
